    ...
    function getQuoteAtTick(
        ...
        if (sqrtRatioX96 <= type(uint128).max) {
            ...
            quoteAmount = baseToken < quoteToken
                ? MulDivLib.mulDiv(ratioX192, baseAmount, 1 << 192)
                : MulDivLib.mulDiv(1 << 192, baseAmount, ratioX192);
        } else {
            uint256 ratioX128 = MulDivLib.mulDiv(sqrtRatioX96, sqrtRatioX96, 1 << 64);
            quoteAmount = baseToken < quoteToken
                ? MulDivLib.mulDiv(ratioX128, baseAmount, 1 << 128)
                : MulDivLib.mulDiv(1 << 128, baseAmount, ratioX128);
        }
    }
